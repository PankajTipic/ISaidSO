...
//     { icon: TrendingUp, label: 'Polls', path: '/polls' },
...
//     <>
//       {/* Top Navigation Bar */}
//       <div className="sticky top-0 z-50 glass-card bg-background/95 border-b border-border/50">
...
//           {/* Mobile Menu Dropdown - Moved INSIDE the sticky container for correct positioning */}
//           <AnimatePresence>
//             {mobileMenuOpen && (
//               <motion.div
...
import { Button } from '@/app/components/ui/button';
import { Input } from '@/app/components/ui/input';
import { currentUser } from '@/app/data/mockData';           // ← keep if you still use mock
import { motion, AnimatePresence } from 'framer-motion';    // note: was 'motion/react' → usually framer-motion
import { useAppSelector } from '@/app/store/hooks';
...
export function TopNav({ 
  ...
  // Hide navbar completely for admin users and admin routes
  if (isAdmin || isAdminPage) {
    return null;
  }

  // ── Normal user navbar below ──────────────────────────
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false);
  const [isDark, setIsDark] = useState(true);
          ...
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="hidden md:flex items-center gap-2">
                ...
                >
                  <AvatarImage 
                    src={user?.avatar_url || user?.avatar || currentUser.avatar} 
                    alt={user?.username || currentUser.username} 
                  />
                  <AvatarFallback>
                    {(user?.username || currentUser.username)?.[0] || '?'}
                  </AvatarFallback>
                </Avatar>
              </div>
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">
                        Level {currentUser.level} • {currentUser.accuracy}% accurate
                      </p>
                    </div>
                  </div>
