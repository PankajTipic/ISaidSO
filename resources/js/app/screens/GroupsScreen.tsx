...
//               <TabsList className="bg-muted/50 p-1 h-10 md:h-12 rounded-xl">
//                 <TabsTrigger value="discover"
//                   //  className="px-4 md:px-6 py-1 md:py-2 rounded-md md:rounded-lg text-[10px] md:text-sm"
//                   className="px-4 md:px-6 py-2 rounded-lg text-sm"
//                 >Discover</TabsTrigger>
//                 <TabsTrigger value="my-groups"
//                   //  className="px-4 md:px-6 py-1 md:py-2 rounded-md md:rounded-lg text-[10px] md:text-sm"
//                   className="px-4 md:px-6 py-2 rounded-lg text-sm"
//                 >My Groups</TabsTrigger>
//               </TabsList>
...
//                   className="h-10 md:h-10 pl-9 bg-muted/30 border-muted-foreground/20 focus:bg-background transition-all text-sm"
//                 />
//               </div>
//             </div>
...
//                               {/* <h3 className="font-bold text-sm md:text-lg truncate text-foreground"> */}
...
//                                 className="md:hidden h-8 px-4 rounded-xl font-semibold text-xs shadow-md"
//                                 style={{
//                                   background: 'linear-gradient(135deg, #a855f7 0%, #ec4899 100%)',
//                                 }}
//                               >
...
//                           <p style={{ color: '#555' }} className="text-muted-foreground mb-2 md:mb-4 text-sm md:text-sm line-clamp-2 leading-relaxed">
//                             {group.description}
//                           </p>

//                           <div className="flex items-center gap-3 text-xs md:text-xs font-medium text-muted-foreground/70">
//                             <div className="flex items-center gap-1">
//                               <Users style={{ color: '#555' }} size={10} className="md:w-3.5 md:h-3.5" />
//                               <span style={{ color: '#555' }}>{group.memberCount} members</span>
//                             </div>
//                             <span className="opacity-30">•</span>
//                             <span style={{ color: '#555' }}>Created {new Date(group.createdAt).toLocaleDateString()}</span>
//                           </div>
//                         </div>
...
export function GroupsScreen() {
  ...
  const handleJoinGroup = async (groupId: number, groupName: string, isPrivate: boolean, isPending: boolean = false) => {
    if (isGuest) {
      toast.info('Please log in to join groups', {
        action: { label: 'Log In', onClick: () => navigate('/auth') }
      });
    ...
    if (isPending) {
      toast.info('Your request is on pending');
      return;
    }
  ...
  /* ── Group card renderer ── */
  const renderGroupCard = (group: Group, index: number, isMine: boolean) => {
    const pal = getPalette(index);
    return (
      <motion.div
        key={group.id}
        className="glass-card rounded-2xl p-4 md:p-5 border border-border/50 hover:border-primary/20 transition-all shadow-sm overflow-hidden"
        style={{
          borderLeftWidth: 3,
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center justify-between md:justify-start gap-3 mb-1 md:mb-2">
              <div className="flex items-center gap-2 min-w-0">
                ...
                >
                  <Users size={14} style={{ color: pal.border }} />
                </div>
                ...
                >
                  {group.isPrivate ? (
                    <Lock size={9} style={{ color: pal.badgeText }} />
                  ) : (
                    <Globe size={9} style={{ color: pal.badgeText }} />
                  )}
                  <span className="text-[9px] font-semibold" style={{ color: pal.badgeText }}>
                    {group.isPrivate ? 'Private' : 'Public'}
                  </span>
                </div>
              </div>

              {/* Mobile action button — plain purple, no gradient */}
              {isMine ? (
                <Button
                  size="sm"
                  variant="outline"
                  className="md:hidden h-8 px-4 rounded-xl font-semibold text-xs"
                ...
                !group.isMember && !isGuest && (
                  <Button
                    onClick={() => handleJoinGroup(group.id, group.name, group.isPrivate, group.pendingRequest)}
                    size="sm"
                    className={`md:hidden h-8 px-4 rounded-xl font-semibold text-xs text-white border-0 ${group.pendingRequest ? 'opacity-50' : ''}`}
                    style={{ background: '#a855f7' }}
                  >
                    {group.pendingRequest ? 'Pending' : (group.isPrivate ? 'Request' : 'Join')}
                  </Button>
                )
              )}
            </div>
  ...
  lineHeight: '1.3', 
            ...
            <div className="flex items-center gap-3 text-xs font-medium">
              <div
                ...
                style={{ background: pal.badge }}
              >
                <Users size={10} style={{ color: pal.badgeText }} />
                <span style={{ color: pal.badgeText }} className="font-semibold">
  ...
  color: '#000000',
          ...
          {/* Desktop action button — plain purple, no gradient */}
          <div className="hidden md:flex justify-end shrink-0">
            {isMine ? (
              <Button
                size="lg"
                variant="outline"
                className="px-10 rounded-full font-bold glass-card shadow-sm transition-all active:scale-95"
                style={{ borderColor: '#a855f7', color: '#a855f7' }}
                onClick={() => handleViewGroup(group.id)}
              >
                View
              ...
              !group.isMember && !isGuest && (
                <Button
                  onClick={() => handleJoinGroup(group.id, group.name, group.isPrivate, group.pendingRequest)}
                  size="lg"
                  className={`px-10 rounded-full font-bold shadow-sm transition-all active:scale-95 text-white border-0 ${group.pendingRequest ? 'opacity-50' : ''}`}
                  style={{ background: '#a855f7' }}
                >
                  {group.pendingRequest ? 'Pending' : (group.isPrivate ? 'Request to Join' : 'Join')}
                </Button>
              )
            )}
          </div>
        ...
        <div className="max-w-5xl mx-auto px-2 md:px-4 py-4 md:py-8">
          ...
          <Tabs value={selectedTab} onValueChange={setSelectedTab} className="space-y-3 md:space-y-6">
            ...
            <div className="flex items-center gap-2">
              ...
              {/* MOBILE: search icon → expands to input (hides tabs while open) */}
              <div className="flex md:hidden items-center gap-2 w-full">
                {showSearch ? (
                  /* Expanded search input */
                  <div className="relative flex-1 animate-in fade-in zoom-in duration-150">
                    <Search
                  ...
                  /* Search icon button + tabs side by side */
                  <>
                    <button
                      onClick={() => setShowSearch(true)}
                      className="h-9 w-9 flex items-center justify-center rounded-full border border-gray-200 dark:border-white/10 bg-white dark:bg-white/5 hover:border-[#a855f7]/50 transition-all shrink-0"
                    >
              ...
              {/* DESKTOP: tabs + search input always visible */}
            ...
            <TabsContent value="my-groups" className="mt-0">
              ...
              >
                {loading ? (
                  ...
                  filteredGroups.map((group, index) => renderGroupCard(group, index, true))
                )}
              </motion.div>
            </TabsContent>
          </Tabs>
