...
const SPIRITUAL_WORDS = [
    { text: 'अहम् ब्रह्मास्मि', lang: 'Sanskrit' },
    { text: 'Divine', lang: 'English' },
    { text: 'A God', lang: 'English' },
    { text: 'Özüm İlahi', lang: 'Turkish' },
    { text: 'God', lang: 'English' },
    { text: 'من تو شدم', lang: 'Farsi' },
    { text: '我即真理', lang: 'Chinese' },
    { text: 'ஆத்மா பிரம்மம்', lang: 'Tamil' },
    { text: 'తత్త్వమసి', lang: 'Telugu' },
    { text: 'তত্ত্বমসি', lang: 'Bengali' },
    { text: 'Soul', lang: 'English' },
    { text: 'Cosmos', lang: 'English' },
    // { text: 'आत्मा', lang: 'Hindi' },
    { text: 'Spirit', lang: 'English' },
    { text: 'Eternal', lang: 'English' },
];

type OrientationType = 'horizontal' | 'vertical' | 'diagonal-up' | 'diagonal-down';
type ShapeType = 'circle' | 'triangle' | 'cross' | 'star' | 'diamond' | 'hexagon';
...
const VIBRANT_COLORS = [
    '#a855f7', '#c026d3', '#7c3aed', '#ec4899',
    '#f472b6', '#818cf8', '#38bdf8', '#fb923c',
...
const SHAPE_TYPES: ShapeType[] = [
    'circle', 'triangle', 'cross', 'star', 'diamond', 'hexagon',
];

// ─── Build floating items ─────────────────────────────────────────────────────

function buildItems(): FloatingItem[] {
    return Array.from({ length: 35 }, (_, i) => {
        const word = SPIRITUAL_WORDS[i % SPIRITUAL_WORDS.length];
        return {
            id: i,
            text: word.text,
            x: 3 + Math.random() * 90,
            ...
            fontSize: 11 + Math.floor(Math.random() * 12),
...
function rotationForOrientation(o: OrientationType): string {
    switch (o) {
        case 'vertical': return 'rotate(-90deg)';
        case 'diagonal-up': return 'rotate(-45deg)';
...
function ShapeSVG({ type, size, color }: { type: ShapeType; size: number; color: string }) {
    ...
    if (type === 'circle') {
        return (
            <svg width={s} height={s} style={baseStyle}>
                <circle
                    cx={s / 2} cy={s / 2} r={h}
    ...
    if (type === 'cross') {
        return (
            <svg width={s} height={s} style={baseStyle}>
                <line x1={s / 2} y1="2" x2={s / 2} y2={s - 2} stroke={color} strokeWidth="1.8" />
                <line x1="2" y1={s / 2} x2={s - 2} y2={s / 2} stroke={color} strokeWidth="1.8" />
            </svg>
        );
    }
    ...
    if (type === 'diamond') {
        const pts = `${s / 2},3 ${s - 3},${s / 2} ${s / 2},${s - 3} 3,${s / 2}`;
        return (
            <svg width={s} height={s} style={baseStyle}>
                <polygon points={pts} fill="none" stroke={color} strokeWidth="1.5" />
            </svg>
    ...
    if (type === 'hexagon') {
        const pts = Array.from({ length: 6 }, (_, k) => {
            ...
            return `${s / 2 + h * 0.9 * Math.cos(angle)},${s / 2 + h * 0.9 * Math.sin(angle)}`;
        }).join(' ');
        return (
            <svg width={s} height={s} style={baseStyle}>
                <polygon points={pts} fill="none" stroke={color} strokeWidth="1.3" />
            </svg>
        );
    }
...
export default function LoginScreen() {
    ...
    const { isLoading, isError, message, isAuthenticated, user, isGuest } = useAppSelector(
        (state) => state.auth,
    );
    ...
    useEffect(() => {
        ...
        const access = params.get('access_token');
        const refresh = params.get('refresh_token');
        if (error) toast.error(msg || 'Google login failed');
        if (access && refresh) {
            // Important: Clear guest status when a real user logs in
            localStorage.removeItem('isGuest');
            localStorage.setItem('access_token', access);
            localStorage.setItem('refresh_token', refresh);
            
            dispatch(checkAuthStatus()).unwrap().then((userData) => {
                toast.success('Logged in successfully');
            }).catch((err) => {
    ...
    useEffect(() => {
        ...
        // Skip redirection if we're already on a protected page? 
        // No, this is LoginScreen, we definitely want to redirect AWAY.
        
        if (!user.is_profile_completed && !isGuest) { 
            navigate('/profile-setup', { replace: true }); 
            return; 
        }
    ...
    const validateForm = () => {
        let isValid = true;
        if (isRegister && !name.trim()) { triggerError('name', 'Name is required'); isValid = false; }
        if (!email.trim()) { triggerError('email', 'Email is required'); isValid = false; }
        else if (!/\S+@\S+\.\S+/.test(email)) { triggerError('email', 'Invalid email format'); isValid = false; }
        if (!password) { triggerError('password', 'Password is required'); isValid = false; }
        else if (password.length < 8) { triggerError('password', 'Password must be at least 8 characters'); isValid = false; }
        if (isRegister && !country.trim()) { triggerError('country', 'Country is required'); isValid = false; }
        if (isRegister && !city.trim()) { triggerError('city', 'City is required'); isValid = false; }
        if (isRegister && password !== passwordConfirmation) { triggerError('passwordConfirmation', 'Passwords do not match'); isValid = false; }
        return isValid;
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Error is already handled in authSlice and shown via isError/message
        }
    };
    ...
    return (
        <div
            className="min-h-screen flex items-center justify-center bg-white font-sans px-4"
        ...
        >
                ...
                {ITEMS.map((item) => (
                    <motion.div
                        ...
                        style={{
                            ...
                            left: `${item.x}%`,
                            top: `${item.y}%`,
                            transform: rotationForOrientation(item.orientation),
                            whiteSpace: 'nowrap',
            ...
            <div
                className="flex flex-col lg:flex-row items-center justify-center gap-8 lg:gap-16 w-full max-w-[980px]"
            ...
            >
                ...
                <motion.div
                    ...
                    className="flex flex-col lg:pb-10 text-center lg:text-left max-w-[480px]"
                >
                    <h1
                        className="text-5xl font-bold italic font-serif tracking-tight leading-none"
                        style={{
                    ...
                    <p className="mt-4 text-2xl text-[#1c1e21] leading-snug font-normal max-w-[380px]">
                        {isRegister
                            ? 'Join and share your predictions with friends.'
                            : 'Connect with friends and see what they predicted.'}
                    </p>
                </motion.div>
                ...
                <motion.div
                    ...
                    animate={{ opacity: 1, y: 0 }}
                    transition={{ duration: 0.4 }}
                    className="w-full max-w-[396px] space-y-3"
                >
                    <div
                        className="bg-white rounded-2xl p-5 border border-transparent"
                        style={{ boxShadow: '0 0 0 1.5px rgba(168,85,247,0.15), 0 8px 32px rgba(168,85,247,0.08)' }}
                    >
                        <form className="space-y-3" onSubmit={handleSubmit}>
                                    ...
                                    >
                                        <motion.div animate={shakeAnimation(shake.name)}>
                                            <Input
                                                ...
                                                placeholder="Full Name"
                                                className={`h-14 text-base rounded-md border-gray-300 bg-white focus:border-[#1877f2] focus:ring-[#1877f2] ${errors.name ? 'border-red-500' : ''}`}
                                            />
                                            ...
                                            <div>
                                                <motion.div animate={shakeAnimation(shake.country)}>
                                                    <Input
                                                        ...
                                                        placeholder="Country"
                                                        className={`h-14 text-base rounded-md border-gray-300 ${errors.country ? 'border-red-500' : ''}`}
                                                    />
                                                </motion.div>
                                            ...
                                            <div>
                                                <motion.div animate={shakeAnimation(shake.city)}>
                                                    <Input
                                                        ...
                                                        placeholder="City"
                                                        className={`h-14 text-base rounded-md border-gray-300 ${errors.city ? 'border-red-500' : ''}`}
                                                    />
                                                </motion.div>
                            ...
                            <motion.div animate={shakeAnimation(shake.email)}>
                                <Input
                                    ...
                                    className={`h-14 text-base rounded-md border-gray-300 focus:border-[#1877f2] ${errors.email ? 'border-red-500' : ''}`}
                                />
                            ...
                            <motion.div animate={shakeAnimation(shake.password)}>
                                <div className="relative">
                                    <Input
                                        ...
                                        placeholder="Password"
                                        className={`h-14 text-base rounded-md border-gray-300 pr-16 focus:border-[#1877f2] ${errors.password ? 'border-red-500' : ''}`}
                                    />
                                    {password && (
                                        <button
                                            type="button"
                                            onClick={() => setShowPassword(!showPassword)}
                                            className="absolute right-3 top-1/2 -translate-y-1/2 text-sm font-semibold text-[#1c1e21] hover:text-gray-500"
                                        >
                                            {showPassword ? 'Hide' : 'Show'}
                                        </button>
                                    )}
                                    ...
                                    >
                                        <motion.div animate={shakeAnimation(shake.passwordConfirmation)}>
                                            <Input
                                                ...
                                                placeholder="Confirm Password"
                                                className={`h-14 text-base rounded-md border-gray-300 ${errors.passwordConfirmation ? 'border-red-500' : ''}`}
                                            />
                            ...
                            <button
                                type="submit"
                                disabled={isLoading}
                                className="w-full h-12 text-white text-base font-bold rounded-xl transition-opacity hover:opacity-90 flex items-center justify-center"
                            ...
                            <button
                                ...
                                className="flex items-center justify-center gap-2 w-full h-12 border border-gray-300 rounded-md bg-white hover:bg-gray-50 text-sm font-semibold text-[#1c1e21] transition-colors mb-2"
                            >
                            ...
                            <a
                                href={`${host}/api/auth/google`}
                                className="flex items-center justify-center gap-2 w-full h-12 border border-gray-300 rounded-md bg-white hover:bg-gray-50 text-sm font-semibold text-[#1c1e21] transition-colors"
                            >
                                <svg className="w-5 h-5" viewBox="0 0 24 24">
                            ...
                            <div className="border-t border-purple-100 pt-4 flex justify-center">
                                <button
                                    type="button"
                                    onClick={() => { setIsRegister(!isRegister); setErrors({}); }}
                                    className={`px-6 py-2.5 rounded-xl font-bold text-sm transition-opacity hover:opacity-90 ${isRegister ? 'text-purple-500 hover:bg-purple-50' : 'text-white'}`}
                                    style={isRegister ? {} : { background: 'linear-gradient(135deg, #a855f7 0%, #ec4899 100%)' }}
                                >
                ...
                {showVerificationModal && (
                    ...
                    >
                        ...
                        >
                            ...
                            >
                                <svg className="w-8 h-8" fill="none" stroke="url(#grad)" viewBox="0 0 24 24">
                                    <defs>
                            ...
                            <p className="text-gray-500 mb-4">
                                ...
                                <span className="font-semibold text-[#1c1e21]">{email}</span>
                            </p>
                            ...
                            <button
                                ...
                                className="w-full text-white font-bold rounded-xl h-12 hover:opacity-90 transition-opacity"
                                style={{ background: 'linear-gradient(135deg, #a855f7 0%, #ec4899 100%)' }}
                            >
                                Got it!
                            </button>
