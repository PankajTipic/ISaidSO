...
export default function UsersTab() {
    ...
    const handleToggleBlock = async (userId: number) => {
        ...
        try {
            const res = await postAuth(`/api/admin/users/${userId}/block`);
            toast.success(res.message || 'Action completed');
            loadUsers(users?.current_page || 1);
        } catch (err: any) {
            toast.error(err.response?.data?.message || 'Action failed');
        } finally {
            setConfirmLoading(false);
        ...
        return (
            <div className="mt-5 flex justify-center items-center gap-3">
                <button
                    disabled={users.current_page === 1 || loading}
                    onClick={() => loadUsers(users.current_page - 1)}
                    className="px-3 py-1.5 rounded-lg border border-indigo-200 bg-indigo-50 text-xs font-medium text-indigo-700 hover:bg-indigo-100 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
                >
                    ← Previous
                </button>
                <span className="text-xs text-slate-500">
                    Page <strong>{users.current_page}</strong> of <strong>{users.last_page}</strong>
                </span>
                <button
                    disabled={users.current_page === users.last_page || loading}
                    onClick={() => loadUsers(users.current_page + 1)}
    ...
    return (
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm">
            ...
            {/* Header — indigo tint */}
            <div className="px-5 py-4 border-b border-indigo-100 bg-indigo-50 rounded-t-2xl">
                <h2 className="font-semibold text-indigo-900">User Management</h2>
                <p className="text-xs text-indigo-400 mt-0.5">{users?.total ?? '—'} total registered users</p>
            </div>
                ...
                <div className="flex flex-col sm:flex-row gap-3 mb-4">
                    <div className="relative flex-1">
                        ...
                        <Input
                            ...
                            value={userSearch}
                            onChange={e => setUserSearch(e.target.value)}
                            className="pl-8 h-9 text-sm border-indigo-200 bg-indigo-50/50 focus:bg-white placeholder:text-indigo-300"
                        />
                        {userSearch && (
                            <button
                ...
                {(userSearch || userStatusFilter !== 'all') && (
                    <div className="flex items-center gap-2 mb-3 text-xs text-slate-500">
                        <span>
                            ...
                            <strong className="text-slate-700">{users?.total ?? 0}</strong> users
                        </span>
                        <button
                            onClick={() => { setUserSearch(''); setUserStatusFilter('all'); }}
                        ...
                        <table className="w-full text-left text-sm">
                            <thead>
                                <tr className="bg-indigo-50 border-b border-indigo-100">
                                    <th className="px-4 py-3 text-left text-[11px] font-semibold text-indigo-500 uppercase tracking-wider">User</th>
                                    <th className="px-4 py-3 text-left text-[11px] font-semibold text-indigo-500 uppercase tracking-wider">Role</th>
                                    <th className="px-4 py-3 text-left text-[11px] font-semibold text-indigo-500 uppercase tracking-wider">Status</th>
                                    <th className="px-4 py-3 text-right text-[11px] font-semibold text-indigo-500 uppercase tracking-wider">Actions</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-50">
                                {filteredUsers.length > 0 ? filteredUsers.map((user, idx) => (
                                    <tr
                                        key={user.id}
                                        className={`transition-colors hover:bg-indigo-50/50 ${idx % 2 === 0 ? 'bg-white' : 'bg-slate-50/30'}`}
                                    >
                                        {/* User cell */}
                                        <td className="px-4 py-3.5">
                                            <div className="flex items-center gap-3">
                                                <div className="w-9 h-9 rounded-full overflow-hidden bg-indigo-100 border border-indigo-200 flex items-center justify-center flex-shrink-0">
                                                    {user.avatar_url ? (
                                                        <img src={user.avatar_url} alt="" className="w-full h-full object-cover" />
                                                    ) : (
                                                ...
                                                <div>
                                                    <p className="font-medium text-slate-900">{user.name || '—'}</p>
                                                    <p className="text-xs text-slate-400">@{user.username || '—'}</p>
                                                    <p className="text-[10px] text-indigo-400 mt-0.5">{user.email || '—'}</p>
                                                </div>
                                            </div>
                                        </td>
            ...
            <Dialog open={!!confirmAction} onOpenChange={() => !confirmLoading && setConfirmAction(null)}>
                <DialogContent className="bg-white border-slate-200 shadow-xl max-w-sm">
                    <DialogHeader>
                        <DialogTitle className="flex items-center gap-2">
                            {confirmAction?.currentBlocked ? (
                                <><CheckCircle size={18} className="text-emerald-600" /> Unblock User</>
                            ) : (
                                <><Ban size={18} className="text-red-500" /> Block User</>
                            )}
                        </DialogTitle>
                    </DialogHeader>
